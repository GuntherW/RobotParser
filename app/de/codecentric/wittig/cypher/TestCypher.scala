...
object TestCypher extends App {
  ...
  def insertRelations = {
    ...
    //    b.foreach(println)
    val ss = wortZuWort.map(_._2.schluesselwort).collect { case regex(a, b) => a -> b }
    wortZuWort.filter(_._2.schluesselwort.matches(regexString)).map { case (s, a) => (s, a) }
    ss.toList.foreach(println)
    println("fertig")
  }
